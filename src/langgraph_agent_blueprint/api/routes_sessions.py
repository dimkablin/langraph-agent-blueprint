...
def list_sessions(request: Request) -> list[SessionListItemDTO]:
    ...
    project_root = deps.config.project_root
...
def get_session_events(session_id: Annotated[str, Path(pattern=RUNTIME_ID_PATTERN)], request: Request) -> list[RuntimeEventDTO]:
    runtime = request.app.state.runtime
    snapshot = runtime.dependencies.session_storage.load_session(runtime.dependencies.config.project_root, session_id)
...
def list_child_runs(session_id: Annotated[str, Path(pattern=RUNTIME_ID_PATTERN)], request: Request) -> list[ChildRunListItemDTO]:
    runtime = request.app.state.runtime
    records = runtime.dependencies.session_storage.list_child_runs(runtime.dependencies.config.project_root, session_id)
...
def get_child_run(
    ...
    runtime = request.app.state.runtime
    record = runtime.dependencies.session_storage.load_child_run(runtime.dependencies.config.project_root, session_id, child_run_id)
...
def export_session(
    ...
    runtime = request.app.state.runtime
    deps = runtime.dependencies
    snapshot = deps.session_storage.load_session(deps.config.project_root, session_id)
...
def get_session(session_id: Annotated[str, Path(pattern=RUNTIME_ID_PATTERN)], request: Request) -> SessionDetailDTO:
    runtime = request.app.state.runtime
    snapshot = runtime.dependencies.session_storage.load_session(runtime.dependencies.config.project_root, session_id)
    child_records = runtime.dependencies.session_storage.list_child_runs(runtime.dependencies.config.project_root, session_id)
